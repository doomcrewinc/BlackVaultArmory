  ...
  try {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
        ...
        <Link href="/vault" className="text-[#00C2FF] text-sm hover:underline">
          Back to vault
        </Link>
      </div>
    );
          ...
          <div className="flex items-end gap-3">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                <span
                  className={`text-xs px-2 py-0.5 rounded border font-mono uppercase ${typeBadge}`}
                >
                  {typeLabel}
                </span>
                ...
                {firearm.nfaClass && firearm.nfaClass !== "NONE" && (
                  <span className="rounded border border-[#F5A623]/40 px-2 py-0.5 text-xs text-[#F5A623]">
                    {NFA_CLASS_LABELS[firearm.nfaClass as NfaClass] ??
                      firearm.nfaClass}
                    {firearm.mgRegistry
                ...
                <RoundCountBadge
                  roundCount={firearm.rangeSessions.reduce(
                    (sum, session) => sum + session.roundsFired,
                    0,
                  )}
                />
              </div>
              <h1 className="text-2xl font-bold text-vault-text leading-tight">
                {firearm.name}
              </h1>
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
          ...
          <div className="bg-vault-surface border border-vault-border rounded-lg p-4">
            ...
            <p className="text-sm font-mono text-vault-text">
              {firearm.currentValue != null && firearm.currentValue !== 0
                ? formatCurrency(firearm.currentValue)
                : "—"}
            </p>
            {gainLoss != null && (
              <p
                className={`text-[10px] mt-0.5 font-mono ${gainLoss >= 0 ? "text-[#00C853]" : "text-[#E53935]"}`}
              >
                {gainLoss >= 0 ? "+" : ""}
        ...
        <div className="grid lg:grid-cols-[minmax(0,1fr)_320px] gap-6">
              ...
              {firearm.builds.length === 0 ? (
                <div className="bg-vault-surface border border-vault-border rounded-lg p-10 text-center">
                  ...
                  <p className="text-xs text-vault-text-muted mb-4 max-w-xs mx-auto">
                    Create a build to start configuring accessories and
                    attachments for this firearm.
                  </p>
                  <Link
                    href={`/vault/${id}/builds/new`}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {firearm.builds.map((build) => {
                    const filledSlots = build.slots.filter(
                      (s) => s.accessoryId,
                    );
                    return (
                      <div
                      ...
                      >
                        ...
                        <div className="mb-4">
                          <p className="text-xs text-vault-text-faint mb-2">
                            {filledSlots.length} of {build.slots.length} slot
                            {build.slots.length !== 1 ? "s" : ""} filled
                          </p>
          ...
          <MaintenanceSection
            ...
            lastMaintenanceDate={
              firearm.lastMaintenanceDate?.toISOString() ?? null
            }
