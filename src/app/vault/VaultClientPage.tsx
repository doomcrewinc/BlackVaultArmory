...
// Presentation, not domain — the badge palette stays local to this page.
...
function FirearmCard({
  ...
  const typeLabel = FILTER_LABELS[firearm.type] ?? firearm.type;
  const activeBuild = firearm.activeBuild;
  const accessoryCount =
    activeBuild?.slots?.filter((s) => s.accessoryId).length ?? 0;
    ...
    <div className="bg-vault-surface border border-vault-border rounded-lg overflow-hidden hover:border-[#00C2FF]/30 transition-colors group flex flex-col">
      ...
      <div className="h-40 bg-vault-bg relative overflow-hidden border-b border-vault-border">
        ...
        <div className="absolute top-2 right-2">
          <span
            className={`text-[10px] px-2 py-0.5 rounded border font-mono uppercase bg-vault-bg/80 backdrop-blur-sm ${typeBadge}`}
          >
            {typeLabel}
          </span>
        </div>
      ...
      <div className="p-4 flex flex-col flex-1">
        ...
        {/* Serial */}
        <div className="mb-3">
          <p className="text-[10px] text-vault-text-faint uppercase tracking-widest mb-0.5">
            Serial
          </p>
          <p className="text-xs font-mono text-vault-text-muted">
        ...
        {activeBuild && !editMode && (
          <div className="mb-3 px-2 py-1.5 rounded bg-vault-bg border border-vault-border">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-xs text-[#00C853] font-medium truncate">
                {activeBuild.name}
              </span>
            </div>
          ...
          <div className="mt-auto flex items-center justify-between pt-3 border-t border-vault-border">
            ...
            {activeBuild ? (
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  router.push(`/vault/${firearm.id}/builds/${activeBuild.id}`);
                }}
        ...
        {editMode && editBuilds && (
          <div className="mt-3 pt-3 border-t border-vault-border">
            <p className="text-[10px] uppercase tracking-widest text-vault-text-faint mb-2">
              Builds
            </p>
            {editBuilds.length === 0 ? (
              ...
              <div className="space-y-1">
                {editBuilds.map((build) => {
                  ...
                  return (
                    <div
                      key={build.id}
                      className="flex items-center gap-2 py-1.5 px-2 rounded bg-vault-bg border border-vault-border group/build"
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[10px] text-vault-text-faint">
                          {accessoriesOnBuild.length} accessories
                        </p>
                      </div>
                      {build.isActive && (
                        <span className="text-[10px] px-1.5 py-0.5 rounded bg-[#00C853]/10 text-[#00C853] border border-[#00C853]/20 shrink-0">
                          Active
                        </span>
                      )}
                      <button
...
export function VaultClientPage({
  ...
  const [buildsByFirearm, setBuildsByFirearm] = useState<
    Record<string, Build[]>
  >({});
  ...
  useEffect(() => {
    const url = sectionSlug
      ? `/api/firearms?section=${encodeURIComponent(sectionSlug)}`
      : "/api/firearms";
    fetch(url)
      .then((r) => r.json())
      .then((data) => {
  ...
  async function handleDeleteBuild(deleteAccessories: boolean) {
    ...
    try {
      ...
      // Refresh firearms list (same scope the page was loaded with)
      ...
      const newBuilds = await fetch(
        `/api/builds?firearmId=${deleteTarget.build.firearmId}`,
      )
        .then((r) => r.json())
        .then((d) => (Array.isArray(d) ? d : []));
      setBuildsByFirearm((prev) => ({
        ...prev,
        [deleteTarget.build.firearmId]: newBuilds,
      }));
  ...
  const counts = FILTER_TYPES.reduce(
    (acc, t) => {
      acc[t] =
        t === "ALL"
          ? firearms.length
          : firearms.filter((f) => f.type === t).length;
      return acc;
    },
  ...
  return (
    <div className="min-h-full">
      <PageHeader
        ...
        subtitle={
          subheading ??
          `${firearms.length} firearm${firearms.length !== 1 ? "s" : ""} in inventory`
        }
        actions={
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <h3 className="text-lg font-semibold text-vault-text mb-2">
              {activeFilter === "ALL"
                ...
                : `No ${FILTER_LABELS[activeFilter]} firearms`}
            </h3>
              ...
              {deleteTarget.accessories.length > 0 ? (
                <>
                  <p className="text-sm text-vault-text-muted mb-3">
                    ...
                    <span className="text-vault-text font-medium">
                      {deleteTarget.accessories.length} accessor
                      {deleteTarget.accessories.length !== 1 ? "ies" : "y"}
                    </span>{" "}
                    assigned to it:
                  </p>
                  <div className="bg-vault-bg border border-vault-border rounded-lg p-3 mb-4 space-y-1 max-h-32 overflow-y-auto">
                    {deleteTarget.accessories.map((a) => (
                      <p key={a.id} className="text-xs text-vault-text-muted">
                        • {a.name}
                      </p>
                    ))}
                  </div>
                  <p className="text-sm text-vault-text-muted mb-4">
                    What would you like to do with these accessories?
                  </p>
                    ...
                    >
                      ...
                      <div>
                        <p className="text-sm font-medium text-vault-text">
                          Keep accessories in vault
                        </p>
                        <p className="text-xs text-vault-text-muted">
                          Accessories stay in your inventory, just unassigned
                        </p>
                      </div>
                    </button>
                    <button
                    ...
                    >
                      ...
                      <div>
                        <p className="text-sm font-medium text-[#E53935]">
                          Delete accessories too
                        </p>
                        <p className="text-xs text-vault-text-muted">
                          Permanently removes accessories from your vault
                        </p>
                      </div>
                    </button>
                  </div>
                ...
                <>
                  <p className="text-sm text-vault-text-muted mb-6">
                    This build has no accessories assigned. Delete it?
                  </p>
                  <button
                    disabled={deleting}
                    onClick={() => handleDeleteBuild(false)}
